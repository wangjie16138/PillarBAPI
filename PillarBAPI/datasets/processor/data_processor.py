...
class DataProcessor(object):
    def __init__(self, processor_configs, point_cloud_range, training, num_point_features):
        ...
        self.mode = 'train' if training else 'study_file'
    ...
    def transform_points_to_voxels(self, data_dict=None, config=None):
        # 初始化点云转换成pillar所需要的参数
        if data_dict is None:
            # [432,496,1]体素网格大小
            grid_size = (self.point_cloud_range[3:6] - self.point_cloud_range[0:3]) / np.array(config.VOXEL_SIZE)
            self.grid_size = np.round(grid_size).astype(np.int64)#四舍五入
            self.voxel_size = config.VOXEL_SIZE
        ...
        if self.voxel_generator is None:
            ...
            self.voxel_generator = VoxelGeneratorWrapper(
                ...
                num_point_features=self.num_point_features, # 点云特征维度
                max_num_points_per_voxel=config.MAX_POINTS_PER_VOXEL, #32
                max_num_voxels=config.MAX_NUMBER_OF_VOXELS[self.mode], # 16000
            )
        # 创建实例，调用generate方法获取voxels, coordinates, num_points
        """
        voxels:pillar数据
        coordinates:pillar坐标
        num_points:生成体素内的有效点云数
        假设点云数据为N*4，则voxels维度为[M,32,4],M个体素，每个体素最大32个点，每个点四个特征
        coordinates维度为[M,3],num_points维度为[m,]
        """
        points = data_dict['points']
