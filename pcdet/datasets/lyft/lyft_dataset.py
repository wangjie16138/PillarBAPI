...
def create_lyft_info(version, data_path, save_path, split, max_sweeps=10):
    ...
    assert version in ['trainval', 'one_scene', 'study_file']

    if version == 'trainval':
        train_split_path = split_path / 'train.txt'
        val_split_path = split_path / 'val.txt'
    elif version == 'study_file':
        train_split_path = split_path / 'study_file.txt'
        val_split_path = None
    ...
    train_lyft_infos, val_lyft_infos = lyft_utils.fill_trainval_infos(
        ...
        test='study_file' in version, max_sweeps=max_sweeps
    )

    if version == 'study_file':
        print('study_file sample: %d' % len(train_lyft_infos))
