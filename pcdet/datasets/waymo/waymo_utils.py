# origin_OpenPCDet PyTorch Dataloader and Evaluation Tools for Waymo Open Dataset
...
def generate_labels(frame, pose):
    ...
    for i in range(len(laser_labels)):
        ...
        dimensions.append([box.length, box.width, box.height])  # lwh in unified coordinate of origin_OpenPCDet
...
def convert_range_image_to_point_cloud(frame, range_images, camera_projections, range_image_top_pose, ri_index=(0, 1)):
    ...
    for c in calibrations:
        ...
        for cur_ri_index in ri_index:
            ...
            if len(c.beam_inclinations) == 0:  # pylint: disable=g-explicit-length-study_file
