...
class NuScenesDataset(DatasetTemplate):
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        if self.dataset_cfg.VERSION == 'v1.0-study_file':
...
def create_nuscenes_info(version, data_path, save_path, max_sweeps=10, with_cam=False):
    ...
    assert version in ['v1.0-trainval', 'v1.0-study_file', 'v1.0-mini']
    if version == 'v1.0-trainval':
        train_scenes = splits.train
        val_scenes = splits.val
    elif version == 'v1.0-study_file':
    ...
    train_nusc_infos, val_nusc_infos = nuscenes_utils.fill_trainval_infos(
        ...
        test='study_file' in version, max_sweeps=max_sweeps, with_cam=with_cam
    )

    if version == 'v1.0-study_file':
        print('study_file sample: %d' % len(train_nusc_infos))
